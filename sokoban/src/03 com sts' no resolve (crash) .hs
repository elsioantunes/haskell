...
solver t0 semaf st = do
    let set = setNew (ordSt st)
    bfs 1 set

  where 
    -- bfs :: Int -> Set A -> IO (Maybe A)
    bfs iter set = do
        ...
        case r of
            (set2, Nothing) -> bfs (iter+1) set2
            (_, Just x)  -> return (Just x)


    -- loopIO :: [A] -> IO (Maybe A)
    -- loopIO :: [Async (Maybe a)] -> IO (Maybe a)
    loopIO set sts' [] = do 
        let set2 = updateSet set sts'
        return (set2, Nothing)
        
    loopIO set sts' (x:xs) = do
        
        -- r <- resolve x
        r <- wait x
        case r of 
            (Nothing, Nothing) -> loopIO set sts' xs
            (Just x, Nothing)  -> loopIO set (x:sts') xs
            (Nothing, Just x)  -> return (set, Just x)

    
    -- resolve :: A -> IO (Maybe A)
    resolve set (m, s) = case funcSucess m s of
        Nothing -> return (Nothing, Nothing)
        Just x  -> go x
      where
        go x | member set x = return (Nothing, Nothing)
             | isgoal x     = return (Nothing, Just x)
             | otherwise    = return (Just x, Nothing)
    
    -- subloop :: [Async (Maybe A)] -> [A] -> IO (Maybe A)
    subloop set asyncs [] = loopIO set [] asyncs
    subloop set asyncs (x:xs) = do
----------------------------------------------------
        withAsync (resolve set x) $ \a ->
            subloop set (a:asyncs) xs
{----------------------------------------------------
        q <- obtemTicket semaf
