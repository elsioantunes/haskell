  ...
  where -- escopo solver
    bfs iter visit sts  = do
      ...
      where  -- escopo bfs
        ...
        loop1 m = do
            vis <- listMyVar visit
            asyncLoop vis (particionaThresh p2 m) 

              where
                -- asyncLoop :: [Jobs] -> IO Box                                   -- main loop list        
                asyncLoop vis = go []       where
                    go asyncs []       = waitLoop asyncs
                    go asyncs (xs:xss) = 
                        withAsync (subloop vis xs) $ \a -> 
                            go (a:asyncs) xss 
                ...
                waitLoop = go (setNew st) where
                    ...
                    go stSet (xs:xss) = do
                        r <- wait xs
                        case r of                                                   
                            (Just cm, _, _) -> return (Just cm, Empty)
                            (Nothing, stSet', vis') ->  do
                                putMyVarS visit vis'
                ...
                subloop vis m = 
                  ...
                  where
                    go vis stSet' []  = return (Nothing, stSet', vis)
                    go vis stSet (x:xs) = do
                        case (funcSucess x) of
                            Goal cm     -> return (Just cm, Empty, [])
                            Invalid     -> go vis stSet xs
                            Factible cm -> do
