...
solver t0 semaf st = bfs 1 (setNew $ ordSt st)
  where 
    bfs iter set = do
        ...
        r <- subloop [] [(m, s) | s  <- tolist set, m  <- actions] set
            
        case r of
            (Nothing, set') -> bfs iter' set'
    ...
    subloop asyncs as =  go asyncs as where
        go asyncs [] = \set' -> loopIO set' asyncs
        go asyncs (x:xs) = \set' -> do

----------------------------------------------------
            withAsync (uncurry funcSucess x) $ \a ->
                go (a:asyncs) xs set'
{----------------------------------------------------}
...
testIO n = do
    ...
    print $ search (div n 2) [1,2..n]
    t1     <- getCurrentTime

    print $ search1 (div n 2) [1,2..n]
    t2     <- getCurrentTime

    print $ search2 (div n 2) [1,2..n]
    t3     <- getCurrentTime
...
search1 _ [] = False
search1 e (x:xs) | x == e = True
                 | otherwise = search1 e xs
