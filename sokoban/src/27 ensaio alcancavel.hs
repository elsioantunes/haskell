...
-}                                        
-- alcancavel :: State -> IO (Maybe b)
alcancavel st = do
    explr  <- newMyVar (player st)
    nivls  <- newMyVar st
    ret    <- newMyVar st
    bfs 1 nivls explr ret

  where
    bfs iter nivls explr ret = do
        if iter > 150 then do return Nothing
        else do 
            sts <- listMyVar nivls
            case sts of
                [] -> do 
                    sts <- listMyVar ret
                    -- print (iter, map (aplica . pat) sts)
                    print iter
                    foldMap (print . explora . pat) sts
                    return Nothing
                _ -> do
            
                    resetMyVar nivls

                    r <- loop [(m, s) | s  <- sts, m  <- actions]

                    case r of 
                        Nothing -> bfs (iter+1) nivls explr ret
                        Just x  -> return (Just x)
                    ...
                    loop (x:xs) = do
                        case (funcSucess x) of
                            ...
                            Factible cm -> do
                                ...
                                look <- lookupMyVar explr (player cm)

                                if look then
                                    loop xs

                                else do
                                    putMyVar explr (player cm)
                                    putMyVar nivls cm
...
explora :: [Moves] -> MovType State
explora m = go m where
    go [] = Factible testSt
    go (m:ms) = 
        case go ms of
            Invalid -> Invalid
