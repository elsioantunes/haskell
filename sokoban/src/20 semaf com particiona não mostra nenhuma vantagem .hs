...
solver t t0 st p2 p3 dbug = do
    ...
    semaf    <- newSemaf p3
    bfs 1 semaf visit [st] 
   
  where -- escopo solver
    bfs iter semaf visit sts  = do
        ...
        if iter > 150 then do return Nothing 
            ...
            if t == 1 then do
                ...
                case r of
                    ...
                    (Nothing, stSet') -> bfs iter' semaf visit (tolist stSet')
      ...
      where  -- escopo bfs
        ...
        loop1 m = 
            asyncLoop (particionaThresh p2 m) 
              ...
              where
                asyncLoop :: [Jobs] -> IO Box                                   -- main loop list        
                asyncLoop = go (setNew st) []       where
                    go stSet asyncs []       = waitLoop stSet asyncs
                    go stSet asyncs (xs:xss) = do
{----------------------------------------------------
...
----------------------------------------------------}
                        ...
                        if False then do
                            ...
                            withAsync res $ \a -> 
                                go stSet (a:asyncs) xss 
                        else do 
                            r <- subloop stSet xs
                            case r of                                                   
                                (Just cm, _) -> return r
                                (Nothing, stSet') -> 
                                    go (merge stSet stSet') asyncs xss
----------------------------------------------------

                waitLoop :: Set State -> [Async Box] -> IO Box
                waitLoop stSet = go stSet where
...
taskIO t p2 = do
    t0 <- getCurrentTime
    solver t t0 testSt 1334 p2 False
    t1 <- getCurrentTime
    print (diffUTCTime t1 t0)
